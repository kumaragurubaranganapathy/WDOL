            ...
            if(event.keyCode === undefined || event.keyCode === 13){
                ...
                if(component.get('v.singleSelectitem')){
                    ...
                    options.forEach(function(element,indice) {
                        ...
                        if(indice === index)   {
                            options[indice]['isSelected'] = !checkboxselection.isSelected;
                        }                     
                        else{
                            options[indice]['isSelected'] = false;
                        }
                        component.set('v.itemObj',checkboxselection);
                        if(options[indice]['isSelected']){
                            
                            if (options[indice].Value === undefined) {
                                selectionEvent.setParams({"selectedItem" : options[indice].Name});
                            } else {
                                selectionEvent.setParams({"selectedItem" : options[indice].Value});
                            }
                        }
                    });
                    
                    selectionEvent.fire(); 
                    //console.log(options[indice]);
                }
